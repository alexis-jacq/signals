...
SM = Variable(torch.Tensor([0,0])).unsqueeze(0)
LM = Variable(torch.Tensor([0,0])).unsqueeze(0)
...
def update(signal):
    ...
    signal = Variable(torch.Tensor([signal,0]).float()).unsqueeze(0)

    SM = 0.9*SM + 0.9*signal
    state = torch.abs(SM - LM)
    print('state '+str(state.data))
    LM = 0.99*LM + 0.01*signal
